  ...
  <div class="flex-grow text-gray-800 lg:ml-20 md:ml-20 sm:ml-20">
    <header class="flex items-center h-20 px-6 sm:px-10 bg-white">
      ...
      <div class="flex flex-shrink-0 items-center ml-auto">
        <button class="inline-flex items-center p-2 hover:bg-gray-100 focus:bg-gray-100 rounded-lg">
          ...
          <span class="font-semibold"><?php echo $user->getNomUser();?></span>
          <span class="text-sm text-gray-600"><?php echo $user->getNomUser();?></span>
          </div>
    ...
    <div class="flex flex-col space-y-6 md:space-y-0 md:flex-row justify-between">
        ...
        if(isset($_POST['deleteall'])){
            Article::deletteAllArticles();
            // $sql = "DELETE FROM article_tags WHERE Id_Article IN (SELECT Id_Article FROM Archive)";
            // mysqli_query($conn, $sql);


            // $sql1 = "DELETE FROM Article WHERE Id_Article IN (SELECT Id_Article FROM Archive) and id_User = ?";
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        <?php
        $ArchiveArticle = Archive::getArchivesByUser($conn,$user->getIdUser());
 
        foreach ($ArchiveArticle as $result) {
            echo '<div class="w-50 bg-white shadow-md rounded-lg overflow-hidden">
                  <div class="relative">
                    <img src="../../public/' . $result['image_Article'] . '" alt="Article Image" 
                         class="w-full h-40 object-cover">
                    <div class="absolute top-3 right-3 flex space-x-2">
                      <form method="POST" action="" >
                        <button type="submit" name="return" title="return" value="' . $result['id_Article'] . '" class="bg-blue-500 hover:bg-blue-700 text-white p-2 rounded-full shadow">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M18 15l4-4m0 0l-4-4m4 4H3" />
                        </svg>
                        </button>
                        <button type="submit" name="Delete" title="Delete" value="' . $result['id_Article'] . '" class="bg-red-500 hover:bg-red-700 text-white p-2 rounded-full shadow">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  ...
                  <div class="p-4">
                    <h2 class="text-lg font-semibold text-gray-800 mb-2">' . $result['titre_Article'] . '</h2>
                    <p class="text-sm text-gray-500">Publié le : ' . $result['Date_Posting'] . '</p>
                  </div>
                </div>';
        }
    ...
    if (isset($_POST['Delete'])) {
        ...
        $archive->returnArticle($conn,$id_Article);
        $archive->deleteArticle($conn,$id_Article);
        echo "<script>window.location.href = window.location.href;</script>";
    }
